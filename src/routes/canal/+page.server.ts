...
function isValidColumn(colName: string): colName is keyof typeof channel.$inferInsert {
	if (Object.keys(channel).includes(colName)) {
		return true;
	}
	return false;
}
...
export async function load({ url }) {
	const query = db.select().from(channel);
	...
	for (const rule of url.searchParams.getAll('order_by')) {
		...
		if (colName && isValidColumn(colName)) {
			...
			sortConditions.push(orderDirFn(channel[colName]));
		}
	}
